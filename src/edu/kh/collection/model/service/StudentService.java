...
public class StudentService {
	...
	// 						- List 크기가 늘었다 줄었다 하기 때문에 큰 의미가 없음 
	...
	public void ex() {
		...
		studentList.add(new Student()); // 0인덱스
	...
	public void selectAll() {
		...
		if(studentList.isEmpty()) { // studentList가 비어있으면 true반환
			...
			return; // 반환명 없는(void) return 구문은 현재 메소드를 종료하고 호출한 곳으로 돌아감.
					// 자동으로 else문을 쓴것같은 효과
		}
		/*
		// 일반 for문
		...
		*/
		...
		// 향상된 for문 (for each문)
		// -컬랙션, 배열의 모든 요소를 순차적으로 반복 접근할 수 있는 for문
		// (순차적: 0번 인덱스부터 마지막요소까지 인덱스를 1씩 증가)
		
		// [작성법]
		// for(컬렉션 or 배열에서 꺼낸 한개의 요소를 저장할 변수 : 컬랙션명 or 배열명) {}
		int index = 0;
		for(Student std : studentList) {
			//std에는 for문 반복시 0,1,2,... 인덱스 요소들을 한 번씩 저장됨.
	...
	public String removeStudent() throws InputMismatchException{
		...
		if(studentList.isEmpty()) {
			...
			char ch = sc.next().toUpperCase().charAt(0);
					 ...
					 // String.toUpperCase() : 물자열을 대문자로 변경
					 // String.toLowerCase() : 문자열으 소문자로 변경
	...
	public void searchName1() {
		...
		boolean flag = true; // falg가 true인 경우 ==> "검색결과가 없습니다."
